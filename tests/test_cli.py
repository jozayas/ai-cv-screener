...
def test_validate_command_validates_single_yaml_file(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    input_path = tmp_path / "candidate.yaml"
    ...
    class FakeCVGenerationService:
        def __init__(self, output_dir: Path) -> None:
            calls.append(("validate_init", output_dir))

        def validate_files(self, paths: list[Path]) -> list[Path]:
            assert paths == [input_path]
            calls.append(("validate_files", input_path.parent))
            return paths

    monkeypatch.setattr(cli_app_module, "CVGenerationService", FakeCVGenerationService)

    result = runner.invoke(
        cli_app,
        ["--color", "never", "validate", str(input_path)],
    )

    assert result.exit_code == 0
    assert calls == [
        ("validate_init", input_path.parent),
        ("validate_files", input_path.parent),
    ]
    assert result.stdout == f"Validated 1 CV YAML file in {input_path}.\n"


def test_validate_command_validates_yaml_directory(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    input_dir = tmp_path / "content"
    calls: list[tuple[str, Path]] = []
    ...
    class FakeCVGenerationService:
        ...
        def validate_directory(self, directory: Path) -> list[Path]:
            assert directory == input_dir
            calls.append(("validate_directory", directory))
            return [directory / "candidate-1.yaml", directory / "candidate-2.yaml"]

    monkeypatch.setattr(cli_app_module, "CVGenerationService", FakeCVGenerationService)

    result = runner.invoke(
        cli_app,
        ["--color", "never", "validate", str(input_dir)],
    )

    assert result.exit_code == 0
    assert calls == [
        ("validate_init", input_dir),
        ("validate_directory", input_dir),
    ]
    assert result.stdout == f"Validated 2 CV YAML files in {input_dir}.\n"
