...
def test_validate_files_loads_specific_yaml(tmp_path: Path) -> None:
    service = CVGenerationService(output_dir=tmp_path)
    generated_files = service.generate(count=2)

    validated_files = service.validate_files([generated_files[0]])

    assert validated_files == [generated_files[0]]
